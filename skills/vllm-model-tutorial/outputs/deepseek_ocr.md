...
$$\text{KV Cache (MHA)} = 2 \times n_{\text{layers}} \times d_{\text{model}} \times \text{seq_len} \times \text{bytes}$$
...
flowchart LR
    subgraph Input["输入"]
        H[hidden states<br/>[batch, seq, d_model]]
    end

    subgraph Q_Proj["Q 投影"]
        Q_Compress[Q 压缩<br/>d_model → d_q_compress]
        Q_Up[Q 上投影<br/>d_q_compress → n_heads × d_head]
    end

    subgraph KV_Compress["KV 潜在压缩"]
        KV_Latent[KV 压缩<br/>d_model → d_c<br/>W^DKV]
        K_Up[K 上投影<br/>d_c → n_kv_heads × d_head]
        V_Up[V 上投影<br/>d_c → n_kv_heads × d_head]
    end
    ...
    H --> Q_Compress --> Q_Up --> Q_RoPE
    H --> KV_Latent --> K_Up & V_Up
    K_Up --> K_RoPE
    Q_RoPE & K_RoPE --> Score --> SM
    SM & V_Up --> Out
```
...
flowchart TB
    subgraph Input["MoE 输入"]
        X[Token hidden state<br/>[batch, seq, d_model]]
    end

    subgraph Router["路由层 (Gate)"]
        Gate[门控网络<br/>Linear: d_model → n_experts]
        Softmax[Softmax]
    ...
    subgraph Routed["路由专家"]
        E1[Expert 1<br/>FFN: d_model → d_ff → d_model]
        E2[Expert 2]
        E3[Expert 3]
        EN[Expert N]
    end
    ...
    subgraph Combine["组合层"]
        WS[加权求和<br/>Σ g_i · E_i(x)]
    end

    X --> Gate --> Softmax --> TopK
    X --> SE
    X --> E1 & E2 & E3 & EN
    TopK -.->|仅激活 Top-K| E1 & E3 & EN
    E1 & E3 & EN --> WS
    SE --> WS
    WS --> O[输出]
```
